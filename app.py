...
def admin_page():
    if request.method == 'POST':
        ...
        res = Bets.query.filter_by(game=gameid).all()
        for re in res:
            db.session.delete(re)

        gametodel = Games.query.filter_by(id=gameid).first()
        db.session.delete(gametodel)
        db.session.commit()
    ...
    if request.method == 'POST':
        ...
        if teamA == teamB:
            ...
            new_game = Games(team1=teamA, team2=teamB, t1wins=returnA, draw=draw, t2wins=returnB)
             # SQL EXECUTIING BELOW IS:
            # INSERT INTO Games (team1, team2, t1wins, draw, t2wins) VALUES (<teamA>, <teamB>, <returnA>, <draw>, <returnB>)
            #SAID BY TA's TO BE OKAY
